...
export default function CustomerLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Login gagal. Periksa email dan kata sandi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: '#EBDEC5' }}>
      <div className="w-full max-w-md bg-white rounded-xl shadow p-6">
        <h1 className="text-2xl font-bold mb-2" style={{ fontFamily: 'var(--font-playfair), serif', color: '#214929' }}>Login Pelanggan</h1>
        <p className="text-sm text-gray-600 mb-6">Masuk untuk melanjutkan pemesanan.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Kata sandi" />
          </div>
          <div className="text-right text-sm">
            <Link href="/forgot-password" className="underline" style={{ color: '#214929' }}>
              Lupa kata sandi?
            </Link>
          </div>
        ...
        <div className="mt-6 text-sm text-center">
          Belum punya akun?{' '}
          <Link href="/signup" className="underline" style={{ color: '#214929' }}>Daftar</Link>
        </div>
      </div>
    </div>
  );
