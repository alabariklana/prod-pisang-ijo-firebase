...
export function CartItem({ item, onUpdateQuantity, onRemove }) {
  const calculateTotal = () => Number(item.price || 0) * Number(item.quantity || 1);

  return (
    <div className="flex items-center justify-between p-3 border rounded-lg">
      <div className="flex-1">
        <h4 className="font-medium text-green-700">{item.name || 'Produk'}</h4>
...
/**
 * Shopping cart component with full functionality
 * @param {Object} props - Component props
 * @param {Array} props.cart - Array of cart items
 * @param {Function} props.onUpdateQuantity - Function to update item quantity
 * @param {Function} props.onRemoveItem - Function to remove item from cart
 * @param {Function} props.onClearCart - Function to clear entire cart
 * @returns {JSX.Element}
 */
