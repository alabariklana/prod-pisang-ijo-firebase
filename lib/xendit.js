...
export function generateExternalId(orderId) {
  ...
  return `PISANGIJO_${orderId}_${timestamp}`;
}
...
export function validateWebhookSignature(rawBody, signature) {
  if (!XENDIT_WEBHOOK_TOKEN || !signature) return false;
  
  try {
    // Import crypto untuk Next.js edge runtime
    const { createHmac } = require('crypto');
    const hash = createHmac('sha256', XENDIT_WEBHOOK_TOKEN)
      .update(rawBody)
      .digest('hex');
